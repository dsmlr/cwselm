function [model] = svdd( X, options )
    ...
    if ~isfield(options,'simf'), options.simf = ''; end
...
%     options.C = 0.1;
    ...
    if strcmp(options.ker,'sim')
        [ n , a , b , c , d ] = FindComponent( X' , X' );
        K = Measures( n , a , b , c , d , options.simf );
    elseif strcmp(options.ker,'euclidean')
        %K = 1-pdist2(X',X','hamming');
        [ n , a , b , c , d ] = FindComponent( X' , X' );
        K = Measures( n , a , b , c , d , 'euclidean' );
    else
